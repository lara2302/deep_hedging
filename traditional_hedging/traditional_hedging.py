...
class Merton_pricer():
    ...
    def mesh_plt(self):
        ...
        #if self.barrier == True:
        #    S_plot = np.exp(self.logS)
        #    V_plot = self.price_matrix
        #else:
        #    S_plot = self.S_vec
        #    V_plot = self.mesh
        ...
        X, Y = np.meshgrid(np.linspace(0, self.T, V_plot.shape[1]), 
                           S_plot)
        ax.plot_surface(Y, X, V_plot, cmap=cm.ocean, rcount = 100, ccount = 100,antialiased=True)
